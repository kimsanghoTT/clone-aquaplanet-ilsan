...
const AsideBar = () => {
  ...
  const eventBoxScrolling = (event) => {
    const eventBoxScroll = event.currentTarget;
  }
  window.addEventListener("scroll", eventBoxScrolling);
  ...
  return (
    <aside className={`${eventList ? "active" : ""}`}>
      ...
      <div className={`event-list-box ${eventList ? "active" : ""}`} onWheel={eventBoxScrolling}>
