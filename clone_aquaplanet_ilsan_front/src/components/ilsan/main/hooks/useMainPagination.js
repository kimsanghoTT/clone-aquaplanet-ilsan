...
const useMainPagination = (sections, paginationRef) => {
  const [navigateSection, setNavigateSection] = useState("section01");
  const scrollingRef = useRef(false);
  const observer = useRef(null);

  useLayoutEffect(() => {
    gsap.fromTo(".main-pagination", { x: -98 }, { x: 0, duration: 1 });
  }, []);
