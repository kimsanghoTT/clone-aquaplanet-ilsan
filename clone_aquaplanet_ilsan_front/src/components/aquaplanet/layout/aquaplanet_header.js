...
const AquaplanetHeader = () => {
    ...
    return(
        <>
            <div className={`aquaplanet-header-overlay-film ${toggleBoxOpen ? "active" : ""}`}></div>
            <header className="aquaplanet-header">
                ...
                <div className="aquaplanet-header-function">
                    <div className="function-join">
                        <a href="/aquaplanet/member/login"><span>로그인</span></a>
                        <a href="/aquaplanet/member/signup"><span>회원가입</span></a>
                    </div>
                    <div className="function-util">
                        <div className="aquaplanet-location-selector">
                            <button type="button" onClick={locationSelector}><span>지역</span></button>
                            <div ref={locationRef} className={`aquaplanet-location-list ${locationSelectorOpen ? "on" : ""}`}>
                                <ul>
                                    {location.map((location, index) => (
                                        <li key={index} className={selectedLocation[location] === "selected" ? "selected" : ""} onClick={() => handleFilter(location)}>{location}</li>
                                    ))}
                                </ul>
                                <button type="button">적용</button>
                            </div>
                        </div>
                        <a className="navigate-home" href="https://www.aquaplanet.co.kr/63/index.do"><span>아쿠아플라넷 홈페이지</span></a>
                        <button className="aquaplanet-header-menu-btn" onClick={handleToggleBox}></button>
                    </div>
                </div>
            </header>
            <div className={`aquaplanet-header-toggle-box ${toggleBoxOpen ? "active" : ""}`}>
